...
func _physics_process(delta):
	...
	if Input.is_action_pressed("crouch"):
		if not is_crouching:
			...
			crouch_tween.tween_property($rotation_helper, "position:y", 0, 0.25)
	elif Input.is_action_just_released("crouch"):
		if is_crouching:
			is_crouching = false
			if crouch_tween.is_running():
				crouch_tween.stop()
				
			crouch_tween = get_tree().create_tween()
			crouch_tween.tween_property($rotation_helper, "position:y", 0.5, 0.25)
...
func _on_ability_grab_click() -> void:
	button_clicked.emit()
